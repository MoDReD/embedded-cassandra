...
class CqlExecutionListenerCassandraSessionTests {
	...
	void shouldHaveUser() {
		assertThat(CqlUtils.getCount(this.cassandraSession, "test.users")).isEqualTo(1);
	}

	@Test
	void shouldNotHaveUser() {
		assertThat(CqlUtils.getCount(this.cassandraSession, "test.users")).isZero();
	}
